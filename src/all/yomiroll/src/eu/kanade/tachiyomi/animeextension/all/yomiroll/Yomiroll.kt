...
class Yomiroll : AnimeHttpSource(), ConfigurableAnimeSource {
    ...
    override val client by lazy {
        super.client.newBuilder().addInterceptor(tokenInterceptor).cookieJar(
    ...
    override fun searchAnimeParse(response: Response): AnimesPage {
        ...
        val total: Int
        val items = if (response.request.url.encodedPath.contains("search")) {
    ...
    private fun getEpisodes(seasonData: SeasonResult.Season): List<SEpisode> {
        val body = client.newCall(GET("$crApiUrl/cms/seasons/${seasonData.id}/episodes"))
            .execute().body.string()
        ...
        return episodes.data.sortedBy { it.episodeNumber }.mapNotNull EpisodeMap@{ ep ->
            SEpisode.create().apply {
                ...
                name = if (ep.episodeNumber > 0 && ep.episode.isNumeric()) {
    ...
    private suspend fun extractVideo(media: Triple<String, String, String>): List<Video> {
        ...
        val subsList = runCatching {
            streams.subtitles?.entries?.mapNotNull { (_, value) ->
    ...
    private suspend fun getStreams(
        ...
        val psshB64 =
            doc.selectFirst("contentprotection[schemeiduri*=edef8ba9-79d6-4ace-a3c8-27dcd51d21ed]")
                ?.text() ?: ""
        ...
        val skipTimes = runCatching {
            client.newCall(GET("https://static.crunchyroll.com/skip-events/production/$skipId.json".toHttpUrlOrNull()!!))
                .execute().parseAs<MediaSegmentsResponse>().let { resp ->
                    listOf(
                        resp.intro,
                        resp.credits,
                        resp.recap,
        ...
        return doc.select(videoSelector).map { element ->
            ...
            docCopy.select(videoSelector).filter { it.attr("height") != quality }
                .forEach { it.remove() }
    ...
    private suspend fun getWidevineKeyExtractor(
        ...
        val headers =
            headersBuilder().add("x-cr-content-id", mediaId).add("x-cr-video-token", videoToken)
                .add("Referer", "https://static.crunchyroll.com/").add("Accept", "*/*")
                .add("Accept-Encoding", "*").build()
    ...
    private fun Anime.toSAnime(anime: SAnime? = null): SAnime = SAnime.create().apply {
        ...
        genre = anime?.genre ?: (series_metadata?.genres ?: movie_metadata?.genres
        ?: genres)?.joinToString { gen -> gen.replaceFirstChar { it.uppercase() } }
        status = anime?.let {
        ...
        description = StringBuilder().apply {
                ...
                if ((subtitle_locales ?: (series_metadata
                        ?: movie_metadata)?.subtitle_locales)?.any() == true || (series_metadata
                        ?: movie_metadata)?.is_subbed == true || is_subbed == true
                ) {
                    append(" Sub")
                }
                if (((series_metadata?.audio_locales ?: audio_locales)?.size
                        ?: 0) > 1 || (series_metadata
                        ?: movie_metadata)?.is_dubbed == true || is_dubbed == true
                ) {
                    append(" Dub")
                }
                appendLine()
    ...
    override fun setupPreferenceScreen(screen: PreferenceScreen) {
        ...
        screen.addEditTextPreference(
            ...
            summary = preferences.password.let {
        ...
        screen.addEditTextPreference(
            ...
            summary = preferences.wvdDevice.let {
