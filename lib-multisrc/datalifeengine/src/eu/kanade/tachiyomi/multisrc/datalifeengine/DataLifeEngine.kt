...
abstract class DataLifeEngine(
        ...
        return when {
            query.isNotBlank() -> {
                if (query.length <
                    ...
                    throw Exception("La recherche est suspendue! La chaîne de recherche est vide ou contient moins de 4 caractères.")
                }
                ...
                if (page == 1) {
                    val postBody =
                        "do=search&subaction=search&story=$cleanQuery".toRequestBody(
                            "application/x-www-form-urlencoded".toMediaType(),
                        )
                    POST("$baseUrl/", body = postBody, headers = postHeaders)
                } else {
                    val postBody =
                        "do=search&subaction=search&search_start=$page&full_search=0&result_from=11&story=$cleanQuery"
                            .toRequestBody(
                                "application/x-www-form-urlencoded".toMediaType(),
                            )
    ...
    private fun animeDetailsParse(
        ...
        return SAnime.create().apply {
            ...
            genre =
                document.select("div.mov-desc span[itemprop=genre] a").joinToString(", ") {
                    it.text()
                }
        }
    }
