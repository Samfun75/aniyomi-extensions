...
abstract class AnimeStream(
    ...
    protected open fun searchAnimeByPathParse(response: Response): AnimesPage {
        val details =
            animeDetailsParse(response.asJsoup()).apply {
    ...
    override fun searchAnimeRequest(
        ...
        return if (query.isNotEmpty()) {
            ...
            GET(
                "$animeListUrl/?page=$page&$multiString&status=${params.status}&type=${params.type}&sub=${params.sub}&order=${params.order}",
            )
        }
    }
    ...
    protected open val filtersHeader =
        when (lang) {
    ...
    protected open val filtersMissingWarning: String =
        when (lang) {
            "pt-BR" -> "Aperte 'Redefinir' para tentar mostrar os filtros"
            else -> "Press 'Reset' to attempt to show the filters"
        }

    protected open val genresFilterText =
        when (lang) {
            "pt-BR" -> "Gêneros"
            else -> "Genres"
        }

    protected open val seasonsFilterText =
        when (lang) {
            "pt-BR" -> "Temporadas"
            else -> "Seasons"
        }

    protected open val studioFilterText =
        when (lang) {
            "pt-BR" -> "Estúdios"
            else -> "Studios"
        }
    ...
    protected open val typeFilterText =
        when (lang) {
            "pt-BR" -> "Tipo"
            else -> "Type"
        }

    protected open val subFilterText =
        when (lang) {
            "pt-BR" -> "Legenda"
            else -> "Subtitle"
        }

    protected open val orderFilterText =
        when (lang) {
            "pt-BR" -> "Ordem"
            else -> "Order"
        }
    ...
    protected open val animeArtistText =
        when (lang) {
            "pt-BR" -> "Estudio"
            else -> "Studio"
        }

    protected open val animeAltNamePrefix =
        when (lang) {
        ...
        SAnime.create().apply {
            ...
            description =
                buildString {
                    ...
                    document
                        .selectFirst(animeAltNameSelector)
                        ?.text()
    ...
    protected open val episodePrefix =
        when (lang) {
            "pt-BR" -> "Episódio"
            else -> "Episode"
        }
    ...
    protected open fun getEpisodeName(
        element: Element,
        epNum: String,
    ) = "$episodePrefix $epNum"
    ...
    protected open fun getHosterUrl(encodedData: String): String {
        val doc =
            if (encodedData.toHttpUrlOrNull() == null) {
                Base64
                    .decode(encodedData, Base64.DEFAULT)
    ...
    protected open fun getVideoList(
        url: String,
        name: String,
    ): List<Video> {
    ...
    protected open fun parseStatus(statusString: String?): Int =
        when (statusString?.trim()?.lowercase()) {
    ...
    protected open fun Element.getInfo(text: String): String? =
        selectFirst("span:contains($text)")
            ?.run {
