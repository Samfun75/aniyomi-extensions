...
3. Configure sparse checkout.
    ...
    # add project folders
    git sparse-checkout add .run common core gradle lib lib-multisrc
    # add a single source
...
| Field | Description |
| ----- | ----------- |
| `name` | The name of the extension. Should be romanized if the site name is not in English. Also used as the source class name unless `qname` is set. |
| `versionCode` | The extension version code. This must be a positive integer and incremented with any change to the code. |
| `qname` | (Optional, defaults to `name`) The class that implements `AnimeSource`, relative to the package `eu.kanade.tachiyomi.animeextension.<lang>.<mysourcename>`. |
| `versionId` | (Optional, defaults to `1`) Bump to give the source a new id, which makes the app treat it as a brand new source. |
| `contentWarning` | (Optional, defaults to `ContentWarning.SAFE`) One of `SAFE`, `MIXED` or `NSFW`. |
| `sourceNames` | (Optional, defaults to `[name]`) The source names an `AnimeSourceFactory` produces. |
| `themePackage` | (Optional) The `lib-multisrc` theme this extension is built on. |
| `overrideVersionCode` | (Optional, defaults to `0`) Added to the theme's `baseVersionCode` to form the version code of a themed extension. |
| `baseUrl` | (Optional) Fills the deep link host and scheme placeholders of a theme's `AndroidManifest.xml`. |

`ContentWarning` is imported from `io.github.samfun75.gradle.api.ContentWarning`.

The extension's version name is generated automatically by concatenating the extension lib version
and `versionCode`. With the example used above, the version would be `17.1`. The `AndroidManifest.xml`
entries the app reads are generated by the `generateExtensionManifest` task, so you only need to write
one yourself for deep linking.
...
### Additional Notes
- A themed extension's version code is calculated as `baseVersionCode + overrideVersionCode`.
...
- Update `versionCode` value in `build.gradle.kts` for individual extensions
- Update `overrideVersionCode` or `baseVersionCode` as needed for all themed extensions
- Reference all related issues in the PR body (e.g. "Closes #xyz")
- Set `contentWarning = ContentWarning.NSFW` in `build.gradle.kts` when appropriate
